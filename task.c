...
__NORETURN
static void coro_exit(void) { _Exit(0); }
...
static void coro_asmctx_make(struct coro_asmctx *ctx, void *stack_base,
...
__NOINLINE
static void coro_switch1(struct coro *from, struct coro *to, void *stack,
                         size_t stack_size) {
  if (to) {
...
#include <sched.h>
static void sched_yield0(void) { sched_yield(); }
#else
#define sched_yield0()
#endif
      ...
      if (cmp < 0) {                                                           \
        ...
        *deleted = node;                                                       \
        if (!node->left && !node->right) {                                     \
          ...
          type *leaf_deleted = 0;                                              \
          if (!node->left) {                                                   \
            node->right = prefix##_delete_first0(node->right, &leaf_deleted);  \
          } else {                                                             \
            node->left = prefix##_delete_last0(node->left, &leaf_deleted);     \
          }                                                                    \
...
static void task_unlock(void) { atomic_store(&task_locker, false); }
...
TASK_EXTERN
int64_t task_id(void) { return task_cur ? task_cur->id : 0; }
...
TASK_EXTERN
void *task_udata(void) { return task_cur ? task_cur->udata : NULL; }

TASK_EXTERN
size_t task_info_scheduled(void) { return task_nyielders; }

TASK_EXTERN
size_t task_info_paused(void) { return task_npaused; }
